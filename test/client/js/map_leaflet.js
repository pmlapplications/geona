...
// import {deLeafletizeZoom} from '../../../src/client/js/map_leaflet';
...
describe('client/js/map_leaflet', function() {
  ...
  describe('addLayer()', function() {
    ...
    it('should have added that layer to _mapLayers', function() {
      ...
      for (let layer of geona.map._mapLayers.getLayers()) {
        if (layer.options.identifier === rrs412.identifier) {
    ...
    it('should display the correct attribution for basemaps', function() {
      ...
      expect(html).to.deep.equal(geona.map.config.basemapLayers[0].layers[0].attribution.onlineResource);
    });
    ...
    it('should remove the previous basemap when a new basemap is added', function() {
      ...
      expect(identifiers).to.include('gebco_08_grid');
  ...
  describe('removeLayer()', function() {
    ...
    it('should set the config borders to be \'none\'', function() {
      expect(geona.map.config.borders).to.deep.equal({identifier: 'none', style: 'none'});
    });
  });
  ...
  describe('setProjection()', function() {
    ...
    it('should change the projection from EPSG:4326 to EPSG:3857', function() {
      ...
      // expect(deLeafletizeZoom(geona.map._map.options.crs)).to.equal('EPSG:3857');
    });
  ...
  describe('changeLayerStyle', function() {
    ...
    it('should keep the zIndex the same before and after changing style', function() {
      ...
      let currentZIndex = rrs412Active.options.zIndex;
  ...
  describe('getLayerServer()', function() {
    ...
    before(function(done) {
      ...
      getLayerServer(geona.geonaServer, 'https://rsg.pml.ac.uk/thredds/wms/CCI_ALL-v3.1-5DAY', 'wms', false, false)
    ...
    it('should add one of the retrieved layers to the map', function() {
      ...
      expect(firstMapLayer.options.identifier).to.equal(wmsLayerServerInfo.layers[0].identifier);
    });
